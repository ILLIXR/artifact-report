...
int main(int argc, char **argv) {
	...
	if(argc < 4) {
		...
		printf(RED "ERROR: ./alignTrajectory <align_mode> <display> <file_gt.txt> <file_est1.txt> ...  <file_est9.txt>\n" RESET);
		std::exit(EXIT_FAILURE);
	}

	int isDisplay = std::stoi(argv[2]);
	...
	for(int i=3; i<argc; i++) {
		...
		if(i>3) {
			...
			std::string fileName = "../../../alignMatrix.txt";
			...
			if (output.is_open())
			{
				...
				for (int row = 0; row < 3; row++)
					for (int col = 0; col < 3; col++)
						output << R_ESTtoGT(row, col) << " ";
				output << std::endl;

				output << "Translation: ";
				for (int col = 0; col < 3; col++)
					output << t_ESTinGT(col) << " ";
				output << std::endl;

				output << "Quaternion: ";
				for (int col = 0; col < 4; col++)
					output << q_ESTtoGT(col) << " ";
				output << std::endl;

				output << "Scale: " << s_ESTtoGT << std::endl;
			}  
			output.close();
	...
	// std::cout << "pose[0].pose " << poses[0][0](0, 0) << " " << poses[0][0](1, 0) << " " << poses[0][0](2, 0) << std::endl;
	// std::cout << "pose[1].pose " << poses[1][0](0, 0) << " " << poses[1][0](1, 0) << " " << poses[1][0](2, 0) << std::endl;

	// translational error of the first frame between groundtruth and estimated
	// double offset_x = poses[0][0](0, 0) - poses[1][0](0, 0);
	// double offset_y = poses[0][0](1, 0) - poses[1][0](1, 0);
	// double offset_z = poses[0][0](2, 0) - poses[1][0](2, 0);
	// std::cout << "offset xyz: " << offset_x << " " << offset_y << " " << offset_z << std::endl;
